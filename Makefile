...
test-stream:
	...
	@curl -s http://localhost:8888/v3/config/global/get > /dev/null && echo "✓ API is responding" || echo "✗ API not responding"
	@echo ""
	@echo "Available paths:"
	@curl -s http://localhost:8888/v3/paths/list 2>/dev/null | grep -o '"name":"[^"]*' | cut -d'"' -f4 || echo "   Cannot retrieve paths"
	@echo ""
